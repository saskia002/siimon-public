...
with open('../4/maria_db_perf_metrics.csv', 'r') as csvfile:
...
# min_cpu = min(item[1] for item in filtered_data)
# max_cpu = max(item[1] for item in filtered_data)
# min_ram = min(item[2] for item in filtered_data)
# max_ram = max(item[2] for item in filtered_data)
...
ax1.grid()
plt.title('MariaDB andmebaasi protsessori ja vahemälu kasutus ajas, 100 virtuaalkasutajat', fontsize=12)
